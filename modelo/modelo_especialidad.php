<?php
  class Modelo_Especialidad{
    ...
    function Modificar_Especialidad($id,$especialidaactual,$especialidadnuevo,$estatus){
      $sql = "call SP_MODIFICAR_ESPECIALIDAD('$id','$especialidaactual','$especialidadnuevo','$estatus')";
      if ($consulta = $this->conexion->conexion->query($sql)){
        if($row = mysqli_fetch_array($consulta)){
            return $id = trim($row[0]);
        }
